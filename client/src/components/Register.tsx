...
export const Register: React.FC = () => {
  ...
  const handleGoogleCredential = useCallback(async (credential: string) => {
    if (!credential) {
      setFormErrors({ general: 'No recibimos credenciales de Google. Intentá de nuevo.' });
      return;
    }
    ...
    try {
      const result = await googleAuth(credential, false);
      if (result.requiresTwoFactor) {
        setFormErrors({
          general:
            'Esta cuenta requiere autenticación de dos pasos. Iniciá sesión para completar la verificación.',
        });
        return;
      }
          ...
          <div className="bg-white rounded-lg shadow-lg border border-gray-200 w-full">
            <div className="p-4 md:p-6">
              ...
              <GoogleLoginButton
                ...
                onUnavailable={(message) => setFormErrors({ general: message })}
